...
class SpatialTemporalCell(nn.Module):
    def __init__(self,
        ...
        if self.use_dynamic_gate:
            self.gru_projection_e = nn.Linear(self.out_channels, self.out_channels * 3)
    ...
    def forward(self, x, cur_extras, prev_hidden=None):
        ...
        if self.use_dynamic_gate:
            extras_r, extras_i, extras_n = self.gru_projection_e(cur_extras).chunk(3, -1)

            # 在节点维度进行广播，使其形状与 input_r 等一致 (B, N, C)
            extras_r = extras_r.unsqueeze(1)
            extras_i = extras_i.unsqueeze(1)
            extras_n = extras_n.unsqueeze(1)
        else:
            extras_r = extras_i = extras_n = 0
        ...
        # 将时间/流量感知项加入到门控计算中
        r = torch.sigmoid(input_r + hidden_r + extras_r + self.bias_r_g)
        i = torch.sigmoid(input_i + hidden_i + extras_i + self.bias_i_g)
        n = torch.tanh(input_n + r * hidden_n + extras_n + self.bias_n_g)
...
class DST_HA(nn.Module):
    def __init__(self, model_cfg):
        ...
        # ablation flags
        self.use_global = model_cfg.get('use_global', True)
        self.use_dynamic_gate = model_cfg.get('use_dynamic_gate', True)
        self.only_global = model_cfg.get('only_global', False)
        self.use_context = model_cfg.get('use_context', True)
        ...
        self.STCell_param_extras = {
            ...
            'use_dynamic_gate': self.use_dynamic_gate,
    ...
    def forward(self, x, y, extras):
        ...
        if self.use_context and extras is not None and len(extras) > 0:
            x_extras_embed, y_extras_embed = self.temporal_embedding(extras)
        else:
            # 无上下文情况：创建零向量
            B = x.size(1)
            device = x.device
            seq_len_x, seq_len_y = x.size(0), y.size(0)
            x_extras_embed = torch.zeros(seq_len_x, B, self.num_rnn_units, device=device)
            y_extras_embed = torch.zeros(seq_len_y, B, self.num_rnn_units, device=device)
